...
#  end
  ...
  defn main_loop(shift, a, x, it) do

    {z, rnorm} = conjgrad(a, x)

    zeta = Nx.add(shift, Nx.divide(1, Nx.dot(x, z)))

    x = Nx.divide(z, enorm(z))          # update_x

    {_,_, _, z, rnorm, zeta, _, _} = while {shift, a, x, z, rnorm, zeta, it, i=1}, Nx.less(i, it) do
      ...
      x = Nx.divide(z, enorm(z))          # update_x
      {shift, a, x, z, rnorm, zeta, it, i + 1}
    end

    {z, rnorm, zeta}
  end

  defn conjgrad(a, x) do
    ...
    {_, z, _, _, _, _} =
      while {a, z, r, p, rho, i = 0}, Nx.less(i, 25) do
        q = Nx.dot(a, p)
